...
final class PuidInitTest: XCTestCase {
  ...
  func testBitsCharSet() throws {
    assert(
      try Puid(bits: 80, chars: Puid.Chars.alpha),
      bits: 85.51,
      bitsPerChar: 5.7,
  ...
  func testBitsCustomAscii() throws {
    ...
    assert(
      try Puid(bits: 100, chars: .custom(chars)),
      bits: 100.32,
      bitsPerChar: 3.46,
      chars: .custom(chars),
      ere: 0.43,
  ...
  func testBitsCustomUtf8() throws {
    ...
    assert(
      try Puid(bits: 64, chars: .custom(chars)),
      bits: 66.61,
      bitsPerChar: 3.7,
      chars: .custom(chars),
      ere: 0.33,
  ...
  func testTotalRiskAscii() throws {
    ...
    assert(
      try Puid(total: 1e9, risk: 1e15, chars: .custom(chars)),
      bits: 109.62,
      bitsPerChar: 3.32,
      chars: .custom(chars),
      ere: 0.42,
